...
void hrms_ultrasonic_init(void) {
  ...
  if (ultrasonic_data_queue == NULL) {
    ultrasonic_data_queue =
        xQueueCreate(ULTRASONIC_QUEUE_LENGTH, sizeof(hrms_ultrasonic_data_t));
...
static bool wait_for_pin(uint32_t port, uint32_t pin, int target_state,
  ...
  while (hrms_gpio_read_pin(port, pin) != target_state) {
...
static bool hrms_ultrasonic_action(hrms_ultrasonic_data_t *data) {
  if (!data)
    return false;
...
static void vUltrasonicTask(void *pvParameters) {
  ...
  for (;;) {
    ...
    hrms_ultrasonic_data_t data;
    if (hrms_ultrasonic_action(&data)) {
